...
class LeCartWoocommerce(LeCartWordpress):
    ...
    def category_import(self, convert, category, categories_ext):
        ...
        id_category = self.import_category_data_connector(self.create_insert_query_connector('terms', category_term),
                                                          True, data['id'])
        # id_category = self.import_category_data_connector( True, data['id'])
        ...
        # term_data = ['sort_order','url_key', 'meta_title']
        for term in term_meta:
            category_term_meta = {
                'term_id': id_category,
                'meta_key': term,
                # 'meta_value': data[term_data[i]]
            }
            term_meta = self.import_category_data_connector(
                self.create_insert_query_connector('termmeta', category_term_meta))

        term_taxonomy = self.import_category_data_connector(
            self.create_insert_query_connector('term_taxonomy', category_term_taxonomy), True, data['id'])
        # if id_category:
    ...
    # query = {
    # 	'type': 'select',
    # 	'query': "SELECT * FROM _DBPRF_category WHERE categories_id = " + to_str(id_parent),
    # }
    # categories = self.get_connector_data(self.get_connector_url('query'), {'query': json.dumps(query)})

    # self.log(categories, "category")
    ...
    def product_import(self, convert, product, products_ext):
        posts = {
            ...
            'post_modified_gmt': convert['description'],
            'post_content_filtered': '',
            # 'post_parent': '0',
            'menu_order': '0',
            'post_type': 'product',
            'post_name': self.sanitize_title(convert['name']),
            # 'post_excerpt': '<img class="alignnone size-medium wp-image-2461" src="{}" />'.format(convert['thumb_image']['url'])
        }
        id_product = self.import_product_data_connector(self.create_insert_query_connector('posts', posts), True,
                                                        convert['id'])
        # thump_post = {
        # 	'post_author': 1,
        # 	'post_date': convert['created_at'],
        # 	'post_date_gmt': convert['updated_at'],
        # 	'post_title': convert['thumb_image']['label'],
        # 	'post_status': 'inherit',
        # 	'comment_status': 'open',
        # 	'ping_status': 'closed',
        # 	'post_name': convert['thumb_image']['label'],
        # 	'post_modified': convert['updated_at'],
        # 	'post_modified_gmt': convert['description'],
        # 	'post_parent': id_product,
        # 	'guid': "http://localhost/wordpress/wp-content/uploads/2020/09/" + convert['thumb_image']['label'],
        # 	'menu_order': '0',
        # 	'post_type': 'attachment',
        # 	'post_mime_type': 'image/jpeg',
        # }
        # id_thump = self.import_product_data_connector(self.create_insert_query_connector('posts', thump_post))
        queries = {
            'type': 'update',
            'query': "UPDATE wp_posts SET guid = 'http://localhost/wordpress/?post_type=product&#038;p={}' WHERE ID = {}".format(
                to_str(id_product), to_str(id_product))
        }
        self.get_connector_data(self.get_connector_url('query'), {'query': json.dumps(queries)})

        # if convert['qty'] > 0:
        # 		stock = 'yes'
        # else:
        # 		stock = 'no'
        # url = "2020/09/" + convert['thumb_image']['label']
        # self.wp_image(path=url,convert=convert)
        if int(convert['qty']) > 0:
            is_stock = 'instock'
        else:
            is_stock = 'outofstock'
        posts_meta = [
            ...
            # 	'meta_value': "2020/09/" + convert['thumb_image']['label'],
            # },
            # {
            # 	'post_id': id_thump,
            ...
            },
            {
                'post_id': id_product,
                'meta_key': '_sku',
                'meta_value': convert['sku']
            },
            {
                'post_id': id_product,
                'meta_key': '_regular_price',
                'meta_value': convert['price']
            },
            {
                'post_id': id_product,
            ...
            },
            {
                'post_id': id_product,
                'meta_key': '_weight',
                'meta_value': convert['weight']
            },
            {
                'post_id': id_product,
                'meta_key': '_length',
                'meta_value': convert['length']
            },
            {
                'post_id': id_product,
                'meta_key': '_width',
                'meta_value': convert['width']
            },
            {
                'post_id': id_product,
                'meta_key': '_height',
                'meta_value': convert['height']
            },
            {
                'post_id': id_product,
    ...
    def customer_import(self, convert, customer, customers_ext):
        ...
        cap = ''
        term_user = ['nick_name', 'first_name', 'last_name', 'description', 'rich_editing', 'syntax_highlighting',
                     'comment_shortcuts', 'admin_color', 'use_ssl', 'show_admin_bar_front',
                     'locale', 'wp_capabilities', 'wp_user_level', 'dismissed_wp_pointers', 'last_update']
        user_data = ['', convert['first_name'], convert['last_name'], '', 'true', 'true', 'false',
                     'fresh', '0', 'true', '', 'a:1:{s:8:"customer";b:1;}', '0', '', convert['updated_at']]
        i = 0
        for term in term_user:
            usermeta = {
                'user_id': customer_id,
                'meta_key': term,
            ...
            i += 1
            customer_term = self.import_category_data_connector(
                self.create_insert_query_connector('usermeta', usermeta))
    ...
    def order_import(self, convert, order, orders_ext):
        # order_id = self.import_category_data_connector(self.create_insert_query_connector('wp_terms', category_term),True, data['id'])
        # order_id = self.get_map_field_by_src(self.TYPE_CUSTOMER, convert['id'], convert['code'])
        post_order = {
            'post_author': 1,
            'post_date': convert['created_at'],
        ...
        order_id = self.import_order_data_connector(self.create_insert_query_connector('posts', post_order), True,
                                                    convert['id'])
        queries = {
            'type': 'update',
            'query': "UPDATE wp_posts SET guid = 'http://localhost/wordpress/?post_type=shop_order&#038;p={}' WHERE ID = {}".format(
                to_str(order_id), to_str(order_id))
        }
        self.get_connector_data(self.get_connector_url('query'), {'query': json.dumps(queries)})
        billingadd = convert['billing_address']
        shippingadd = convert['shipping_address']
        post_meta_order = [
            {
                'post_id': order_id,
            ...
            },
            {
                ...
                'meta_value': convert['total']['amount']
            },
            {
                'post_id': order_id,
            ...
            },
            {
                ...
                'meta_value': billingadd.get('first_name') + ' ' + billingadd.get('last_name') + ' '
                              + billingadd.get('company') + billingadd.get('address_1') + ' ' + billingadd.get(
                    'address_2')
                              + ' ' + billingadd.get('city') + ' ' + str(billingadd['state'].get('state_code')) + ' '
                              + billingadd.get('postcode') + ' ' + str(billingadd['country'].get('country_code'))
            },
            {
                'post_id': order_id,
                'meta_key': '_shipping_address_index',
                'meta_value': shippingadd['first_name'] + ' ' + shippingadd['last_name'] + ' '
                              + shippingadd['company'] + shippingadd['address_1'] + ' ' + shippingadd['address_2']
                              + ' ' + shippingadd['city'] + ' ' + str(shippingadd['state'].get('state_code')) + ' '
                              + shippingadd['postcode'] + ' ' + str(shippingadd['country'].get('country_code'))
            },
            {
                'post_id': order_id,
            ...
            },
            {
                ...
                'meta_value': convert['shipping']['amount']
            },
            {
                'post_id': order_id,
                'meta_key': '_payment_method',
                'meta_value': convert['payment']['method']
            },
            {
                'post_id': order_id,
            ...
            },
            {
                ...
                'meta_value': billingadd['first_name']
            },
            {
                'post_id': order_id,
                'meta_key': '_billing_last_name',
                'meta_value': billingadd['last_name']
            },
            {
                'post_id': order_id,
                'meta_key': '_billing_company',
                'meta_value': billingadd['company']
            },
            {
                'post_id': order_id,
                'meta_key': '_billing_address_1',
                'meta_value': billingadd['address_1']
            },
            {
                'post_id': order_id,
                'meta_key': '_billing_address_1',
                'meta_value': billingadd['address_2']
            },
            {
                'post_id': order_id,
                'meta_key': '_billing_city',
                'meta_value': billingadd['city']
            },
            {
                'post_id': order_id,
                'meta_key': '_billing_state',
                'meta_value': billingadd['state']['state_code']
            },
            {
                'post_id': order_id,
                'meta_key': '_billing_postcode',
                'meta_value': billingadd['postcode']
            },
            {
                'post_id': order_id,
                'meta_key': '_billing_country',
                'meta_value': billingadd['country']['country_code']
            },
            {
                'post_id': order_id,
                'meta_key': '_shipping_first_name',
                'meta_value': shippingadd['first_name']
            },
            {
                'post_id': order_id,
                'meta_key': '_shipping_last_name',
                'meta_value': shippingadd['last_name']
            },
            {
                'post_id': order_id,
                'meta_key': '_shipping_company',
                'meta_value': shippingadd['company']
            },
            {
                'post_id': order_id,
                'meta_key': '_shipping_address_1',
                'meta_value': shippingadd['address_1']
            },
            {
                'post_id': order_id,
                'meta_key': '_shipping_address_1',
                'meta_value': shippingadd['address_2']
            },
            {
                'post_id': order_id,
                'meta_key': '_shipping_city',
                'meta_value': shippingadd['city']
            },
            {
                'post_id': order_id,
                'meta_key': '_shipping_state',
                'meta_value': shippingadd['state']['state_code']
            },
            {
                'post_id': order_id,
                'meta_key': '_shipping_postcode',
                'meta_value': shippingadd['postcode']
            },
            {
                'post_id': order_id,
                'meta_key': '_shipping_country',
                'meta_value': shippingadd['country']['country_code']
            },
            {
                'post_id': order_id,
        ...
        for postterm in post_meta_order:
            self.import_order_data_connector(self.create_insert_query_connector('postmeta', postterm))
        # self.import_order_data_connector(self.create_insert_query_connector('posts', url_order),True, convert['id'])
        for i in range(len(convert['items'])):
            ...
            order_items1_id = self.import_order_data_connector(
                self.create_insert_query_connector('woocommerce_order_items', order_items1))
        i = 0
        if convert['items']:
            order_itemmeta = [
                {
                    'order_item_id': order_items1_id,
                    'meta_key': '_product_id',
                    'meta_value': convert['items'][i]['product']['id']
                },
                {
                    'order_item_id': order_items1_id,
                ...
                },
                {
                    'order_item_id': order_items1_id,
                    'meta_key': '_qty',
                    'meta_value': convert['items'][i]['qty']
                },
                {
                    'order_item_id': order_items1_id,
                    'meta_key': '_tax_class',
                ...
                },
                {
                    ...
                    'meta_value': convert['items'][i]['price']
                },
                {
                    'order_item_id': order_items1_id,
                    'meta_key': '_line_tax',
                ...
                },
                {
                    ...
                    'meta_value': 'a:2:{s:8:"subtotal";a:0:{}s:5:"total";a:0:{}}',
                },
                {
                    'order_item_id': order_items1_id,
            ...
            i += 1
        order_items2 = {
            'order_item_name': convert['shipping']['title'],
            'order_item_type': 'shipping',
            'order_id': order_id
        }
        id_order_items2 = self.import_order_data_connector(
            self.create_insert_query_connector('woocommerce_order_items', order_items2))
        order_item_meta = [
            {
                'order_item_id': id_order_items2,
                'meta_key': 'method_id',
                'meta_value': '',
            },
            {
        ...
        for item in order_item_meta:
            self.import_order_data_connector(self.create_insert_query_connector('woocommerce_order_itemmeta', item))
        # for order in order_items:
        # 		self.import_order_data_connector(self.create_insert_query_connector('woocommerce_order_items', order))
