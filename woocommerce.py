...
class LeCartWoocommerce(LeCartWordpress):
    ...
    def manufacturer_import(self, convert, manufacturer, manufacturers_ext):
        ...
        if convert['thumb_image']['url'] or convert['thumb_image']['path']:
            ...
            image_import_path = self.uploadImageConnector(image_process, self.add_prefix_path(
                self.make_woocommerce_image_path(image_process['path'], self.TYPE_MANUFACTURER),
                self._notice['target']['config']['image_manufacturer'].rstrip('/')))
    ...
    def get_categories_main_export(self):
        ...
        if not categories or categories['result'] != 'success':
            return response_error('could not get manufacturers main to export')
        self.log(categories, "categories")
        return categories
    ...
    def category_import(self, convert, category, categories_ext):
        self.log(convert, "cate_convert")
        self.log(category, "category_import")
        self.log(categories_ext, "cat_ext")
        category_term = {
            'name': convert['name'],
            'slug': convert['name'],
        }
        category_id = self.import_category_data_connector(self.create_insert_query_connector('terms', category_term),
                                                          True, convert['id'])
        ...
        for term in term_meta:
            ...
            self.import_category_data_connector(self.create_insert_query_connector('termmeta', category_term_meta))
        category_term_taxonomy = {
            'term_id': category_id,
            'taxonomy': 'product_cat',
            'description': convert['name'],
        }

        term_taxonomy = self.import_category_data_connector(
            self.create_insert_query_connector('term_taxonomy', category_term_taxonomy), True, convert['id'])

        self.insert_map(self.TYPE_CATEGORY, convert['id'], category_id, convert['code'])
        return response_success(category_id)
    ...
    def product_import(self, convert, product, products_ext):
        self.log(convert, "product_convert")
        self.log(product, "product_product")
        self.log(products_ext, "product_products_ext")
        queries = {
            'type': 'select',
            'query': "select max(ID) from wp_posts"
        }
        max_id = int(
            self.get_connector_data(self.get_connector_url('query'), {'query': json.dumps(queries)})['data'][0][
                'max(ID)'])
        products = {
            'post_author': '1',
            ...
            'ping_status': 'open',
            'post_type': 'product',
            'guid': f"http://localhost/wordpress/?post_type=product&#038;p=#{max_id+1}",
            'post_content': convert['description']
        }

        product_id = self.import_order_data_connector(self.create_insert_query_connector('posts', products), True,
                                                    convert['id'])
        if convert['thumb_image']['url'] or convert['thumb_image']['path']:
            image_process = self.process_image_before_import(convert['thumb_image']['url'],
                                                             convert['thumb_image']['path'])
            image_import_path = self.uploadImageConnector(image_process, self.add_prefix_path(
                self.make_woocommerce_image_path(image_process['path']),
            ...
            query1 = {
                'type': 'update',
                'query': f"UPDATE wp_posts SET post_parent = {product_id} WHERE ID = {thumbnail_id}"
            }
            self.get_connector_data(self.get_connector_url('query'), {'query': json.dumps(query1)})
            query2 = {
                'type': 'insert',
                'query': f"INSERT into wp_postmeta (post_id, meta_key, meta_value) values ({product_id}, '_thumbnail_id', {thumbnail_id})"
            }
            self.get_connector_data(self.get_connector_url('query'), {'query': json.dumps(query2)})


        post_meta_key = ['_edit_last', '_sku', '_regular_price', '_tax_status', '_manage_stock', '_backorders',
                         '_sold_individually', '_weight', '_length', '_width', '_height',
                         '_product_version', '_price', '_stock', '_thumbnail_id']
        post_meta_value = ['1', convert['sku'], convert['price'], 'taxable', 'yes', 'no', 'no',
                           convert['weight'], convert['length'], convert['width'], convert['height'],
                           '4.5.2', convert['price'], convert['qty'], thumbnail_id]
        posts_meta = []
        for i in range(0, len(post_meta_key) - 1):
            params = {
                'post_id': product_id,
                'meta_key': post_meta_key[i],
                'meta_value': post_meta_value[i]
            }
            posts_meta.append(params)

        self.log(posts_meta, "posts_meta")

        for meta in posts_meta:
            self.import_product_data_connector(self.create_insert_query_connector('postmeta', meta), True,
                                               convert['id'])
        ...
        self.import_product_data_connector(
            self.create_insert_query_connector('wc_product_meta_lookup', product_meta_lookup))

        query_product_type = {
            'type': 'select',
            'query': 'SELECT * FROM _DBPRF_term_taxonomy WHERE taxonomy = "product_type"'
        }
        id_product_type = self.select_data_connector(query_product_type, 'id_product_type')['data'][0][
            'term_taxonomy_id']
        for meta in posts_meta:
            self.import_product_data_connector(self.create_insert_query_connector('postmeta', meta), True,
                                               convert['id'])

        id_categories = self.get_map_field_by_src(self.TYPE_CATEGORY, convert['categories'][0]['id'])
        query = {
            'type': 'select',
            'query': 'SELECT * FROM _DBPRF_term_taxonomy WHERE term_id = ' + to_str(id_categories),
        }
        category_term_rela = self.select_data_connector(query, 'category_term_rela')['data']
        term1_relationships = {
            'object_id': product_id,
            'term_taxonomy_id': id_product_type
        }
        self.import_product_data_connector(
            self.create_insert_query_connector('term_relationships', term1_relationships))
        term2_relationships = {
            'object_id': product_id,
            'term_taxonomy_id': category_term_rela[0]['term_taxonomy_id']
        }
        self.import_product_data_connector(
            self.create_insert_query_connector('term_relationships', term2_relationships))
    ...
    def customer_import(self, convert, customer, customers_ext):
        ...
        self.log(customer, "cus")
        self.log(customers_ext, "cus_ext")
        self.clear_target_customers_demo()
        data = convert
        user = {
            ...
            'user_registered': data['updated_at'],
        }
        customer_id = self.import_category_data_connector(self.create_insert_query_connector('users', user))
        self.log(self.create_insert_query_connector('users', user), "insert_query")
        terms = ['nickname', 'first_name', 'last_name', 'last_update', 'billing_address_1', 'billing_city', 'billing_country']
        user_data = ['', convert['first_name'], convert['last_name'], convert['updated_at'], convert['address'],
                     convert['city'], convert['city']['name']]
        for i in range(0, len(terms) - 1):
            meta_data = {
                'user_id': customer_id,
                'meta_key': terms[i],
                'meta_value': user_data[i],
            }
            self.import_category_data_connector(self.create_insert_query_connector('usermeta', meta_data))
            self.log(self.create_insert_query_connector('usermeta', meta_data), "insert_query2")
        self.insert_map(self.TYPE_CUSTOMER, convert['id'], customer_id)
    ...
    def order_import(self, convert, order, orders_ext):
        self.log(convert, "order_convert")
        self.log(order, "order_order")
        ...
        max_id = int(self.get_connector_data(self.get_connector_url('query'), {'query': json.dumps(queries)})['data'][0]['max(ID)'])
        order = {
            'post_author': 1,
            'post_date': convert['updated_at'],
            'post_date_gmt': convert['updated_at'],
            ...
            'guid': f"http://localhost/wordpress/?post_type=shop_order&#038;p=#{max_id+1}"
        }
        order_id = self.import_order_data_connector(self.create_insert_query_connector('posts', order), True,
                                                    convert['id'])

        shipping_address = convert['shipping_address']
        order_meta_key = ["_customer_user","_billing_first_name", "_billing_last_name", "_order_currency", "_order_shipping_tax", "_order_tax",
               "_order_total", "_order_version", "_prices_include_tax", "_shipping_address_index",
               "_cart_discount", "_cart_discount_tax", "_order_shipping", "_payment_method",
               "_payment_method_title", "_shipping_first_name", "_shipping_last_name", "_shipping_address_1"
               "_shipping_company", "_shipping_city", "_shipping_state", "_shipping_postcode",
               "_shipping_country", "_recorded_sales", "_recorded_coupon_usage_counts", "_order_stock_reduced"
               ]
        order_meta_value = [0,shipping_address['first_name'], shipping_address['last_name'], convert['currency'], 0, 0, convert['total']['amount'], '4.5.2', 'no',
                            shipping_address['state'].get('state_code') + " " + shipping_address['country'].get('country_code'),
                            0, 0, convert['shipping']['amount'], convert['payment']['method'] , convert['payment']['method'],
                            shipping_address['first_name'], shipping_address['last_name'], shipping_address['address_1'], shipping_address['company'], shipping_address['city'],
                            shipping_address['state']['state_code'], shipping_address['postcode'], shipping_address['country']['country_code'],
                            'yes', 'yes', 'yes']
        orders_meta = []
        for i in range(0, len(order_meta_key) - 1):
            params = {
                'post_id': order_id,
                'meta_key': order_meta_key[i],
                'meta_value': order_meta_value[i]
            }
            orders_meta.append(params)

        #

        for order in orders_meta:
            self.import_order_data_connector(self.create_insert_query_connector('postmeta', order))
        self.insert_map(self.TYPE_ORDER, convert['id'], order_id, convert['code'])

        for i in range(len(convert['items'])):
            order_items1 = {
                'order_item_name': convert['items'][i]['product']['name'],
                'order_item_type': 'line_item',
                'order_id': order_id,
            }
            order_item_id = self.import_order_data_connector(
                self.create_insert_query_connector('woocommerce_order_items', order_items1))
        id_product = self.get_map_field_by_src(self.TYPE_PRODUCT, convert['items'][i]['product']['id'])
        item_meta_key = ["_product_id", "_variation_id", "_qty", "_tax_class", "_line_subtotal",
                         "_line_subtotal_tax", "_line_total", "_line_tax", "_line_tax_data",
                         "_reduced_stock"]
        item_meta_value = [id_product, 0, convert['items'][i]['qty'], '',
                           '', 0, convert['items'][i]['price'], 0, 'a:2:{s:8:"subtotal";a:0:{}s:5:"total";a:0:{}}',
                           convert['items'][i]['qty']]
        order_itemmeta = []
        for i in range(0,len(item_meta_key)-1):
            item = {
                'order_item_id': order_item_id,
                'meta_key': item_meta_key[i],
                'meta_value': item_meta_value[i]
            }
            order_itemmeta.append(item)

            for meta in order_itemmeta:
                self.import_order_data_connector(self.create_insert_query_connector('woocommerce_order_itemmeta', meta))
        query3 = {
            'type': 'insert',
            'query': f"INSERT into wp_wc_order_product_lookup (order_item_id, order_id, product_id, variation_id, customer_id, date_created, product_qty, product_net_revenue, product_gross_revenue) values ({order_item_id}, {order_id}, {convert['items'][0]['product']['id']}, 0, 1)"
        }
        self.get_connector_data(self.get_connector_url('query'), {'query': json.dumps(query3)})
                    ...
                    if select['result'] == 'success':
                        ...
                        if trid and not is_tranlate:
                            ...
                            if opt_value_id:
                                ...
                                self.import_product_data_connector(
                                    self.create_insert_query_connector('icl_translations', icl_translations_data))
                                self.insert_map(self.TYPE_ATTR_VALUE, None, opt_value_id, 'pa_' + pro_attr_code, None,
                                                slug, language_code)
